;2025-11-25 Ho-Ro:
;convert to Z80-MBC2 iLoad
;changed OUTC:  IOS TXD function
;changed CHKIO: RXD function
;changed INIT:  no UART initialisation
...
ROMEND:         .EQU    $0800           ; End of 2K ROM
RAMBGN:         .EQU    $0800           ; RAM starts after ROM
RAMEND:         .EQU    $1000           ; End of 2K RAM
;
...
;
LST:    CALL TSTNUM                     ;TEST IF THERE IS A #
...
OC3:
        ...
        .else
        MVI  A,1                        ;TXD opcode
        OUT  1                          ;Out to opcode port
        POP  PSW                        ;READY, GET OLD A BACK
        OUT  0                          ;Out to data port
        .endif
        CPI  CR                         ;WAS IT CR?
        RNZ                             ;NO, FINISHED
...
TAB1:                           ;DIRECT COMMANDS
        ...
        DWA  LST                ;  LIST is an assembler pseudo op
...
TAB2:                           ;DIRECT/STATEMENT
        ...
        DWA  IFIF               ;  IF is an assembler pseudo op
...
BYTES_FREE:     .EQU    ROMEND-LSTROM
;
; CHECK THAT THE PROGRAM DOES NOT EXCEED ROM SIZE
;
                .ASSERT $ <= ROMEND

                .DC     RAMBGN-$,$00    ;FILL WITH $00 UTIL RAMBGN
...
STACK:  .EQU RAMEND                     ;STACK STARTS HERE
;
CR      EQU  0DH
LF      EQU  0AH
