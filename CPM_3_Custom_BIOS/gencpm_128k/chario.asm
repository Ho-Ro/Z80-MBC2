...
?CINIT:
  ENDIF
    ...
    DW  NULL$INIT   ;INIT FOR DEVICE 0: nothing to initialize
    DW  NULL$INIT   ;INIT FOR DEVICE 1: nothing to initialize
...
?CI:
  ENDIF
    ...
    DW  NULL$CI     ;DEVICE 1 INPUT: can't read from printer (SPP Adapter)
...
?CIST:
  ENDIF
    ...
    DW  NULL$CIST   ;DEVICE 1 INPUT STATUS: can't read from printer (SPP Adapter)
...
COST1:
    mvi     a, GETSPP$OPC   ; Select GETSPP$OPC opcode (IOS)
    out     STO$OPCD
    in      EXC$RD$OPCD     ; Read it
    ana     a               ; Is SPP enabled?
    jrz     COST1RDY        ; No, jump
    cpi     SPPRDY          ; Yes, is SPP printer ready?
    jrz     COST1RDY        ; Yes, jump
    xra     a               ; No, A = not ready flag
    ret

COST1RDY:
    mvi      a, 0ffH        ; A = 0xFF (ready flag)
    ret
...
CO1:
    ...
    mvi      a, WRSPP$OPC   ; Yes, select WRSPP$OPC opcode (IOS)...
    out     STO$OPCD
    mov      a, c           ; ... and send the current char to the SPP printer
    out     EXC$WR$OPCD
listRet:
    ret
...
@CTBL:
    ...
    DB  'LPT   '            ; SPP Adapter board/parallel printer (DEVICE 1)
    DB  MB$OUTPUT
    DB  BAUD$NONE
