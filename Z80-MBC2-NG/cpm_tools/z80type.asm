...
; A>ASM Z80TYPE or MAC Z80TYPE.ASM
; A>LOAD Z80TYPE
; A>Z80TYPE
;
; Modified for Z80-MBC2 with BIOS-Mod by SvenMb 
; Modified for Z80-MBC2-SIO BIOS by Ho-Ro 
...
RPORT		EQU	00H				; IOS "execute opcode" read port address

	ORG	0100H

	MVI	A,0FEH		; GET VERBOSITY CMD
	OUT	OPCODE
	IN	RPORT		; GET VERBOSITY VALUE FROM IOS
	STA	VERBOSITY	; SAVE IT TO BE RESTORED AT END
...
DONE:
	...
	MVI	A,07EH		; SET VERBOSITY CMD
	OUT	OPCODE
	LDA	VERBOSITY	; GET SAVED VERBOSITY
	OUT	WPORT		; RESTORE VERBOSITY VALUE TO IOS
RETCPM:
	RET			; RETURN TO CP/M
...
;     00 - NMOS CPU
;     FF - CMOS CPU
; 3. RETURN 00 OR FF in REG A
;
	MVI	A,07EH		; SET VERBOSITY CMD
	OUT	OPCODE
	MVI	C,WPORT
	DB	0EDH, 071H	; UNDOCUMENTED OUT (C),<0|0FFH> INSTRUCTION
				; WRITE 00 OR FF TO IOS VERBOSITY BYTE
	MVI	A,0FEH
	OUT	OPCODE
	IN	RPORT		; READ BACK 00 OR FF
	RET
